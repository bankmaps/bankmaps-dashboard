...
const CreateAccountPage = ({ searchParams }) => {
    const token = searchParams.get('token');

    // Check for the token and return an error message if not found
    if (!token) {
        return (
    ...
    // Main UI render (only if token exists)
    return (
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 20px' }}>
            <h1>Create Account</h1>
            {/* Your progress circles */}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '48px' }}>
                {currentStep > 1 && (
                    <button onClick={prevStep} style={{ padding: '12px 28px', background: '#6c757d', color: 'white', border: 'none', borderRadius: '6px' }}>
                        Back
                    </button>
                )}
                {currentStep < 4 ? (
                    ...
                    <button onClick={handleSave} style={{ padding: '12px 28px', background: '#28a745', color: 'white', border: 'none', borderRadius: '6px' }}>
                        Save & Continue
                    </button>
                )}
            </div>
