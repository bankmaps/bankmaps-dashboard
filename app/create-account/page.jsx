...
export default function Page() {
  ...
  const [candidates, setCandidates] = useState({ hmda: [], cra: [], branch: [], fdic: [], ncua: [] });
  const [selectedLenderPerSource, setSelectedLenderPerSource] = useState({ hmda: null, cra: null, branch: null, fdic: null, ncua: null });
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      } catch (e) { console.error('NCUA:', e); }

      // Sort alphabetically by label (case-insensitive)
      const sortAlpha = arr => arr.sort((a, b) => a.label.localeCompare(b.label, 'en', { sensitivity: 'base' }));
      ...
      // Best match for display only (with percentage)
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1:
        return <div><h2>Step 1 – Organization Name</h2><input type="text" value={orgName} onChange={e => setOrgName(e.target.value)} placeholder="e.g. XYZ Savings Bank" style={{width:'100%',padding:'12px',borderRadius:'6px',border:'1px solid #ccc'}} /></div>;

      case 2:
        return (
          <div>
            <h2>Step 2 – Organization Type</h2>
            <select value={selectedOrgType} onChange={e => setSelectedOrgType(e.target.value)} style={{width:'100%',padding:'12px',borderRadius:'6px',border:'1px solid #ccc'}}>
      ...
      case 3:
        return (
          <div>
            ...
            <Select isMulti options={stateOptions} value={stateOptions.filter(opt => selectedStates.includes(opt.value))} onChange={opts => setSelectedStates(opts ? opts.map(o => o.value) : [])} placeholder="Select state(s)..." className="basic-multi-select" classNamePrefix="select" />
          </div>
        );
  ...
  return (
    <div style={{maxWidth:'720px', margin:'0 auto', padding:'40px 20px'}}>
      ...
      <div style={{display:'flex', justifyContent:'space-between', marginTop:'48px'}}>
        {currentStep > 1 && <button onClick={prevStep} style={{padding:'12px 28px', background:'#6c757d', color:'white', border:'none', borderRadius:'6px', cursor:'pointer'}}>Back</button>}
        {currentStep < 4 ? (
