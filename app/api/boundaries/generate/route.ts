    ...
    for (const geo of geographies) {
      ...
      for (const vintage of CENSUS_VINTAGES) {
        try {
          ...
          if (tracts.length > 0) {
            ...
            console.log(`[BOUNDARY] PostGIS union successful: ${boundaryGeoJSON.type}, area: ${totalAland} m²`);
          }

          if (!boundaryGeoJSON) {
            console.log(`[BOUNDARY] No boundary geometry for "${geoName}" vintage ${vintage}`);
            continue;
          }
      ...
      try {
        ...
        if (geoidList.length > 0) {
          ...
          console.log(`[BOUNDARY] No geoids found for "${geoName}"`);
        }
        
      } catch (geoTractsError: any) {
        console.error(`[BOUNDARY] ERROR populating geography_tracts for "${geoName}":`, geoTractsError.message);
        console.error(`[BOUNDARY] Full geography_tracts error:`, geoTractsError);
      }
    }
